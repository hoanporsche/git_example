...
import { save } from '../CategoryApiCaller';
...
class Update extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.category) {
      ...
      this.setState({
        form: {
          id: { value: category.id, valid: true },
          code: { value: category.code, valid: true },
          name: { value: category.name, valid: true },
          picture: { value: category.picture, valid: true },
        }
      })
    }
  }
        ...
        <div id="modal-update" className="modal fade" tabIndex={-1} role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              <div className="modal-header"><h5>Update Category</h5></div>
              <div className="modal-body">
                <CustomInput type="text" name="name" placeholder="Category's name" value={this.state.form.name.value}
                  maxLength={20} onEmittedValue={this.onReceivedValue} wasSubmitted={this.state.wasSubmitted} />
                <CustomInput type="text" name="picture" placeholder="Category's picture" value={this.state.form.picture.value}
