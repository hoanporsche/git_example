...
import { save } from '../ConfigGlobalApiCaller';
...
class Update extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.configGlobal) {
      const { configGlobal } = this.props;
      this.setState({
        form: {
          name: { value: configGlobal.name, valid: true },
          value: { value: configGlobal.value, valid: true },
        }
      })
    }
  }
  ...
  onSubmit = () => {
    if (!this.state.isSubmitting) {
      const { name, value } = this.state.form;
      if (isFormValid([name, value])) {
        ...
        <div id="modal-update" className="modal fade" tabIndex={-1} role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              <div className="modal-header"><h5>Update ConfigGlobal</h5></div>
              <div className="modal-body">
                <CustomInput type="text" name="value" placeholder="ConfigGlobal's value" value={this.state.form.value.value}
