...
import { save } from '../WorkingCalenderApiCaller';
...
class Update extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.workingCalender) {
      const { workingCalender } = this.props;
      this.setState({
        form: {
          id: { value: workingCalender.id, valid: true },
          name: { value: workingCalender.name, valid: true },
          description: { value: workingCalender.description, valid: true },
        }
      })
    }
  }
        ...
        <div id="modal-update" className="modal fade" tabIndex={-1} role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              <div className="modal-header"><h5>Update WorkingCalender</h5></div>
              <div className="modal-body">
                <CustomInput type="text" name="name" placeholder="WorkingCalender's name" value={this.state.form.name.value}
                  maxLength={20} onEmittedValue={this.onReceivedValue} wasSubmitted={this.state.wasSubmitted} />
                <CustomInput type="text" name="description" placeholder="WorkingCalender's description" value={this.state.form.description.value}
