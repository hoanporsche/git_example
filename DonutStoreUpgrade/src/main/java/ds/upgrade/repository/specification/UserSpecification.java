...
public class UserSpecification implements Specification<User> {
  ...
  public Predicate toPredicate(Root<User> root, CriteriaQuery<?> arg1, CriteriaBuilder cb) {
    ...
    if (storeId != null) {
      predicate = cb.and(predicate, cb.equal(root.<Long>get(AppConstants.PARAM.STORE_ID_PARAM).get(AppConstants.PARAM.ID_PARAM), storeId));
    }
    if (startDate != null) {
    ...
    if (roleId != null) {
      predicate = cb.and(predicate,
          cb.equal(root.join(AppConstants.PARAM.ROLES_PARAM).get(AppConstants.PARAM.ID_PARAM), roleId));
    }
    return predicate;
  }
